	...
	for(int j  = 0 ; j < n; j++) 
  	{	
		...
		if(j+idxChild < n-idxChild)
		{
			...
			printf("\nEl hijo [%d] con pid [%d] modificó la matriz: [%d][%d] = [%d]",idxChild,pid,x_sup,y_sup,matriz[x_sup][y_sup]);
			...
			if(n%2 == 0 || idxChild != cantChild -1)
			{
				...
				printf("\nEl hijo [%d] con pid [%d] modificó la matriz: [%d][%d] = [%d]",idxChild,pid,x_inf,y_inf,matriz[x_inf][y_inf]);
			}
		...
		if(idxChild != cantChild -1) //El ultimo uno lo requiere.
		{
			if(j+idxChild+1 < n-idxChild-1)
			{
				...
				printf("\nEl hijo [%d] con pid [%d] modificó la matriz: [%d][%d] = [%d]",idxChild,pid,x_izq,y_izq,matriz[x_izq][y_izq]);
				printf("\nEl hijo [%d] con pid [%d] modificó la matriz: [%d][%d] = [%d]",idxChild,pid,x_der,y_der,matriz[x_der][y_der]);
		 	}
		}
	}
